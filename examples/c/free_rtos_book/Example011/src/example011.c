...
int main( void )
{
   ...
   if( xQueue != NULL ) {
      ...
      xTaskCreate( vSenderTask, "Sender1", configMINIMAL_STACK_SIZE*2, ( void * ) &( xStructsToSend[ 0 ] ), 2, NULL );
      xTaskCreate( vSenderTask, "Sender2", configMINIMAL_STACK_SIZE*2, ( void * ) &( xStructsToSend[ 1 ] ), 2, NULL );

      /* Create the task that will read from the queue.  The task is created with
      priority 1, so below the priority of the sender tasks. */
      xTaskCreate( vReceiverTask, "Receiver", configMINIMAL_STACK_SIZE*2, NULL, 1, NULL );
