...
int main( void )
{
   ...
   xTaskCreate( vEventBitSettingTask, "BitSetter", configMINIMAL_STACK_SIZE, NULL, 1, NULL );

   /* Create the task that waits for event bits to get set in the event
   group. */
   xTaskCreate( vEventBitReadingTask, "BitReader", configMINIMAL_STACK_SIZE, NULL, 2, NULL );

   /* Create the task that is used to periodically generate a software
   interrupt. */
   xTaskCreate( vIntegerGenerator, "IntGen", configMINIMAL_STACK_SIZE, NULL, 3, NULL );
