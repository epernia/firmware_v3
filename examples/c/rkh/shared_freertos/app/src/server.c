...
RKH_TRINT( REQ,     NULL,   server_defer ),
           ...
           RKH_CREATE_BASIC_STATE( server_paused, server_pause, server_resume, RKH_ROOT, NULL );
           ...
           struct Server
{
    ...
    rui32_t ncr[NUM_CLIENTS]; 	/* number of attended requests of every client */
};
...
server_init( Server *const me, RKH_EVT_T *pe )
{
    ...
    rkh_queue_init( &queueReq, ( const void ** )queueReqSto, MAX_SIZEOF_QREQ, CV( 0 ) );
