...
RKH_CREATE_BASIC_STATE( client_idle,  client_delay_req, NULL,  RKH_ROOT, NULL );
...
RKH_TRREG( TOUT_USING,  NULL,   client_end,      &client_idle ),
           ...
           RKH_CREATE_BASIC_STATE( client_paused,  client_pause, client_resume, RKH_ROOT, NULL );
...
client_start( Client *const me, RKH_EVT_T *pe )
{
    ...
    bsp_cli_using( RKH_CAST( StartEvt, pe )->clino,   time / RKH_CFG_FWK_TICK_RATE_HZ );
}
